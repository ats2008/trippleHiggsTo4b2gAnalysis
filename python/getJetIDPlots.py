...
def main():
    ...
    varsToGet+=[
        ...
        'quadjet_2_isTight',
        'quadjet_3_isTight',
        'quadjet_0_isTight2017',
        'quadjet_1_isTight2017',
        'quadjet_2_isTight2017',
        'quadjet_3_isTight2017',
        'quadjet_0_isTight2018',
        'quadjet_1_isTight2018',
        'quadjet_2_isTight2018',
        'quadjet_3_isTight2018'
        ]
    
    allProducts=itertools.product(['L','T'],repeat=4)
    ...
    keys[0]=np.array(keys[0]).reshape(4,4)
    keys[1]=np.array(keys[1]).reshape(4,4)
    for yr in yearsToProcess:
        saveBase=prefixBase+'/'+yr+'/jetIDEfficiencies/'
        ...
        for dset in ['bkg','data','sig']:
            ...
            for ky in rdataFrames[yr][dset]:
                ...
                idMap={'b1':{},'b2':{},'b3':{},'b4':{}}
                idMap['b1']['T'] = varData['quadjet_0_isTight'] >0.2
                idMap['b2']['T'] = varData['quadjet_1_isTight'] >0.2
                idMap['b3']['T'] = varData['quadjet_2_isTight'] >0.2
                idMap['b4']['T'] = varData['quadjet_3_isTight'] >0.2
                idMap['b1']['L'] = varData['quadjet_0_isLoose'] >0.2
                idMap['b2']['L'] = varData['quadjet_1_isLoose'] >0.2
                idMap['b3']['L'] = varData['quadjet_2_isLoose'] >0.2
                idMap['b4']['L'] = varData['quadjet_3_isLoose'] >0.2
                ...
                scoreMaps[dset]=np.array(vals).reshape(4,4)
                ...
                ax.set_xticks(np.arange(4)+0.5,np.arange(4)+0.5,minor=True,color='w')
                ax.set_yticks(np.arange(4)+0.5,np.arange(4)+0.5,minor=True,color='w')
                ax.set_xticks(np.arange(4),labels=keys[0][:,0])
                ax.set_yticks(np.arange(4),labels=keys[1][0,:])
                ax.set_xlabel(dset)
                ax.grid(which='minor')
                ax2 = f.add_axes([ax.get_position().x1+0.005,ax.get_position().y0,0.02,ax.get_position().height])
                plt.colorbar(c,cax=ax2)
                ax2.set_yticks([])
                f.savefig(saveBase+'/selectionEffciency_'+dset+'.png',bbox_inches='tight')
                plt.close(f)
        ...
        f,axSet=plt.subplots(1,2,figsize=(9,4))

        for i,dset in enumerate(['data','bkg']):
            ax=axSet[i]
            significance = scoreMaps['sig']*1e4/np.sqrt(scoreMaps['sig'] + scoreMaps[dset] )
            ...
            plt.colorbar(c,cax=ax2)
            ax2.set_yticks([])
        f.savefig(saveBase+'/significance_'+dset+'.png',bbox_inches='tight')
