...
class classifier :
    ...
    def predict_probaForX(self,dataDict):
        ...
        x=np.array([ dataDict[ky] for ky in self.modelParamters  ] ).reshape(1,-1)
...
class sklearn_gbBDT( sklearn_model ):       
    def __init__(self,modelParamters=None):
        super().__init__(modelParamters)
        #print(self.__dict__)
        #print("HERE")
...
class xgb_bdt( classifier  ):
    ...
    def setModel(self, clf,modelParamters ,verbose=False):
        self.clf = clf
        self.modelParamters = modelParamters
        if verbose:
            for i in self.modelParamters:
                print("\t\t",i)
...
def plot_mvaScores(y_train_pred,W_train,C_train,y_true=None,fname=None):    
    ...
    for kidx in range(nMax):
        ...
        hep.cms.label('Work In Progress',ax=ax[kidx])
...
def plotTPRvsFPR(y_train_pred,y_true,W_train,fname=None):
    ...
    nMax=y_train_pred.shape[1]
    print(Y_train_reduced.shape,y_train_pred.shape)
