...
if __name__=='__main__':
    ...
    prefixBase='/home/aravind/cernbox/work/trippleHiggs/hhhTo4b2gamma/genAnalysis/python/analysis/results/plots/analysis/mar3/correlations/'
    fileDict={}
    ...
    for yr  in yearsToProcess:
        ...
        for ky in rdataFrames[yr]['bkg']:
            ...
            if n<2000:
                ...
                nBkg+=2000
            _dataDict=rdataFrames[yr]['bkg'][ky].Filter('rdfentry_ < 2000').AsNumpy(columns=['weight_binned']+varlist)
            _data.append([_dataDict[ky] for ky in varlist ])
            _weight.append(_dataDict['weight_binned'])
        dataForCorr[yr]['bkg']['x']=np.concatenate(_data,axis=1)
        dataForCorr[yr]['bkg']['weight_binned']=np.concatenate(_weight)
        nStr=str(nBkg)
        _data=[]
        _weight=[]
        for ky in rdataFrames[yr]['sig']:
            _dataDict=rdataFrames[yr]['sig'][ky].Filter('rdfentry_ < '+nStr).AsNumpy(columns=['weight_binned']+varlist)
            _data.append([_dataDict[ky] for ky in varlist ])
            _weight.append(_dataDict['weight_binned'])
        dataForCorr[yr]['sig']['x']=np.concatenate(_data,axis=1)
        dataForCorr[yr]['sig']['weight_binned']=np.concatenate(_weight)
    
    
        _data=[]
        _weight=[]
        for ky in rdataFrames[yr]['data']:
            _dataDict=rdataFrames[yr]['data'][ky].Filter('rdfentry_ < '+nStr).AsNumpy(columns=['weight_binned']+varlist)
            _data.append([_dataDict[ky] for ky in varlist ])
            _weight.append(_dataDict['weight_binned'])
        dataForCorr[yr]['data']['x']=np.concatenate(_data,axis=1)
        dataForCorr[yr]['data']['weight_binned']=np.concatenate(_weight)
