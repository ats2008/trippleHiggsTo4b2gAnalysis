...
for tag in tagList:
    if filemode=='update':
        for cat in ['cat0','cat1','cat2','cat3','cat4','cat5','cat6']:
        ...
        for cat in ['cat0','cat1','cat2','cat3','cat4','cat5','cat6']:
            ...
            histStore[cat+tag]={}
            histStore[cat+tag]['m1jj'] = ROOT.TH1F("m1jj","H1bb mass",300,0.0,300.0 )
            histStore[cat+tag]['m2jj'] = ROOT.TH1F("m2jj","H1bb mass",300,0.0,300.0 )
            histStore[cat+tag]['m1m2'] = ROOT.TH2F("m1jj_m2jj","H1bb , H2bb mass",300,0.0,300.0,300,0.0,300. )
            histStore[cat+tag]['m1m2_evt'] = ROOT.TH2F("m1jj_m2jj_evt","H1bb , H2bb mass",300,0.0,300.0,300,0.0,300. )
            histStore[cat+tag]['mvaNR_v0']=ROOT.TH1F("mvaNR_v0","",44,-1.1,1.1)
            histStore[cat+tag]['mvaNR_v1']=ROOT.TH1F("mvaNR_v1","",44,-1.1,1.1)
            for i in range(4):
                histStore[cat+tag]['mlScore_jet_'+str(i)]=ROOT.TH1F("mlScore_jet_"+str(i),"",44,-1.1,1.1)
            histStore[cat+tag]['n_goodJets']=ROOT.TH1F("n_goodJets","",6,-0.5,5.5)
...
for fname in allFnames:
    ...
    for i in range(maxEvents_):
        ...
        allQuads=hhhSelector.getBJetParisFGG_MHA(eTree,etaMax,pTMin,mlScoreTag='H3SIN61',threshold=-1e3)
        ...
        cat='cat0'
        if tofill['r_HH'] < 40.0 :
            if quad['n_goodJets'] == 4:
                cat = 'cat1'
            elif quad['n_goodJets'] == 3: 
                cat = 'cat2'
            else:
                cat ='cat5'
        else   : 
        ...
        x=[]
        for i in range(4):
            histStore[cat+tag]['mlScore_jet_'+str(i)].Fill(quad['mlScore'][i])
            x.append(quad['mlScore'][i])
            
        nGoodJets= np.array(x) > mlScoreThreashold 
        histStore[cat+tag]['n_goodJets'].Fill(sum(nGoodJets))
        #if tag!='':
