...
if __name__ == "__main__":
    ...
    dataHist=histStore['data2018']['validation_CR']['hgg']['pT'].Clone()
    dataHist.Rebin(rebinC)
    mcHists=[]
    mcHistsToSubstract=[]
    #for tag in ['ggM80Inc', 'ggM80Jbox1bjet','ggM80Jbox2bjet','gJet40ToInf','gJet20To40']:
    for tag in ['ggM80Inc', 'ggM80Jbox1bjet','ggM80Jbox2bjet']:
        mcHists.append(histStore[tag]['validation_CR']['hgg']['pT'].Clone())
        mcHists[-1].Rebin(rebinC)
    for tag in ['ttgJets', 'ttgg', 'tGJets']:
        mcHistsToSubstract.append(histStore[tag]['validation_CR']['hgg']['pT'].Clone())
    ...
    h.SetAxisRange(0.0,2e4, "Y")
    h.SetAxisRange(0.0,400.0, "X")
    h.SetMarkerColor(8)
    h.SetLineColor(4)
    ...
    legend.Draw()
    d.Draw()
    d.SaveAs("dataMCVal.png")
    k=pTDependentScaler.scaleFactorHist.Clone()
    k.SetAxisRange(0.0,700.0, "X")
    k.Draw()
    d.Draw()
    d.SaveAs("ScalefactorVal.png")

    f=ROOT.TFile("pTScaleFactorFileValidation.root","RECREATE")
