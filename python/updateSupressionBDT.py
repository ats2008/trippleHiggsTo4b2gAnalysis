...
if bkgSupressionBDTFile:
    with open(bkgSupressionBDTFile, 'rb') as f:
        modelDict = pickle.load(f)
        supressionModel=clsUtl.xgb_bdt()
        supressionModel.setModel(modelDict['model'],modelDict['variables'])
...
for fname in allFnames:
    ...
    for eid in range(len(testDict)):
        
        mvaScore=-1.5
        tofill=testDict[eid]
        if supressionModel:
            mvaScore=supressionModel.predict_probaForX(tofill)[1]
        print(mvaScore)  
        print(testDict[eid]['predScore'])
        continue
