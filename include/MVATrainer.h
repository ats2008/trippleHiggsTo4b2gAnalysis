...
void MVATrainer::readParameters(string fname)
{
    ...
    if(signalFnames.size() != signalTreeNames.size() )  {  std::cout<<"#Trees != #Files for signal"<<" config error !! \n"; exit(1) ;}
    if(signalFnames.size() != signalWeight.size() )     {  std::cout<<"#Weights ! #Files for signal"<<" config error !! \n"; exit(1) ;}
    if(bkgFnames.size() != bkgTreeNames.size() )        {  std::cout<<"#Trees != #Files for bkg"<<" config error !! \n"; exit(1) ;}
    if(bkgFnames.size() != bkgWeight.size() )           {  std::cout<<"#Weights != #Files for bkg"<<" config error !! \n"; exit(1) ;}
}
